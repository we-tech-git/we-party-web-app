...
export const useEventsStore = defineStore("events", () => {
  ...
  const confirmedStored = localStorage.getItem(
    getScopedKey(CONFIRMED_EVENTS_KEY),
  );
  if (confirmedStored) {
    try {
  ...
  async function toggleSave(event: FeedItem) {
    ...
    localStorage.setItem(
      getScopedKey(SAVED_EVENTS_KEY),
      JSON.stringify(savedEvents.value),
    );
  ...
  async function toggleLike(id: EventId) {
    ...
    localStorage.setItem(
      getScopedKey(LIKED_EVENTS_KEY),
      JSON.stringify(likedEvents.value),
    );
  ...
  async function toggleConfirm(id: EventId) {
    ...
    localStorage.setItem(
      getScopedKey(CONFIRMED_EVENTS_KEY),
      JSON.stringify(confirmedEvents.value),
    );
