<script setup lang="ts">
  ...
  import FeedTrendingMobile from './FeedTrendingMobile.vue'
  ...
  // goToTrendingEvent: migrou pra FeedTrendingMobile.vue (Fase 5, parte 1)
  // junto com o resto do FAB/bottom sheet de tendências mobile.
  ...
  <div class="feed-page">
    ...
    <section class="feed-shell">
      ...
      <main ref="feedMainRef" class="feed-main">
        ...
        <BreadcrumbBack
          v-if="activeNav !== 'home'"
          :current="activeNav === 'top-events' ? t('feed.nav.topEvents') : activeNav === 'favorites' ? t('feed.nav.favorites') : ''"
    ...
    <!-- FAB + bottom sheet de tendências mobile — extraídos pra
         FeedTrendingMobile.vue na Fase 5 do REFACTOR_AUDIT_PLAN.md (1ª
         fatia da decomposição de Feed.vue). `is-mobile` só afeta a
         visibilidade do botão FAB — o bottom sheet, como no original,
         não depende dele (só de `visible`). -->
    <FeedTrendingMobile
      :guest-mode="props.guestMode"
      :is-mobile="isMobile"
      :trends="displayedTrends"
      :visible="showTrendingMobile"
      @update:visible="showTrendingMobile = $event"
    />
  </div>
</template>
