...
<template>
  <div class="feed-page">
            ...
            <div v-if="filterOpen" class="filter-panel">
              <div class="filter-section">
                <div class="filter-section-header">
                  ...
                  <button v-if="userInterestChips.length > 0" class="explore-categories-btn" type="button"
                ...
                <div v-if="activeCategoryLabels.length > 0" class="active-tags">
                  <button v-for="cat in activeCategoryLabels" :key="'tag-' + cat.id" class="active-tag" type="button"
                ...
                <div v-if="showCategorySearch" class="category-search-box">
                  ...
                  <input ref="categorySearchInput" v-model="categorySearchQuery" autocomplete="off"
                    class="category-search-input" placeholder="Ex: Rock, Yoga, Cinema..." type="text">
                  <button v-if="categorySearchQuery" class="search-clear-btn" type="button"
                ...
                <div v-if="showCategorySearch && categorySearchQuery.trim() && !searchingCategories"
                ...
                <span v-if="showCategorySearch && !categorySearchQuery.trim() && allInterestsCache.length > 0"
                ...
                <div v-else-if="displayedCategories.length > 0" class="filter-chips">
                  <button v-for="cat in displayedCategories" :key="cat.id" class="filter-chip"
                    :class="{ active: activeCategories.includes(cat.id) }" type="button"
                    @click="toggleCategory(cat.id)">
                    {{ cat.label }}
                  </button>
                </div>
                <div v-else-if="showCategorySearch && categorySearchQuery.trim() && !searchingCategories"
                ...
                <div v-else-if="!loadingCategories && userInterestChips.length === 0 && !showCategorySearch"
                  class="empty-categories">
              ...
              <div class="filter-section">
                ...
                <div class="filter-chips">
                  <button v-for="d in DATE_CHIPS" :key="d.id" class="filter-chip filter-chip--date"
                    :class="{ active: activeDateFilter === d.id }" type="button" @click="toggleDateFilter(d.id)">
                    {{ d.label }}
                  </button>
                </div>
              </div>
    ...
    <section class="feed-shell">
      <FeedSidebarNav :active="activeNav" class="feed-sidebar" :items="guestFilteredNavItems"
      ...
      <main class="feed-main">
        ...
        <section v-else-if="displayedItems.length > 0" class="cards-stack">
          <FeedCard v-for="(item, index) in displayedItems" :id="item.id" :key="item.id" :banner="item.banner"
            :comments-count="item.commentsCount" :confirmed="item.confirmed" :description="item.description"
            :event-data="item" :guest-mode="props.guestMode" :highlight="activeNav === 'top-events'"
            :host-avatar="item.hostAvatar" :host-name="item.creator.name" :interested="item.interested"
            :interests="item.interests" :is-saved="eventsStore.isSaved(item.id)" :liked="eventsStore.isLiked(item.id)"
            :likes="(item.likes || 0) + (eventsStore.isLiked(item.id) ? 1 : 0)" :location="item.location"
            :matched-interests="item.matchedInterests" :rank="activeNav === 'top-events' ? index + 1 : undefined"
            :schedule="item.schedule" :source-url="item.sourceUrl" :title="item.title"
            @toggle-like="eventsStore.toggleLike(item.id)" @toggle-save="handleToggleSave(item)" />
        </section>

        <div v-if="!loading && hasMore && displayedItems.length > 0 && !isSearching" class="load-more-container">
          <button class="load-more-btn" :disabled="loadingMore" type="button"
