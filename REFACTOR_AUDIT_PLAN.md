...
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟡 Em andamento | Parte 1: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45) (mergeado) · Parte 2: [#46](https://github.com/we-tech-git/we-party-web-app/pull/46) (mergeado) · Parte 3: [#47](https://github.com/we-tech-git/we-party-web-app/pull/47) (mergeado) · Parte 4: [#48](https://github.com/we-tech-git/we-party-web-app/pull/48) (mergeado) · Parte 5: [#49](https://github.com/we-tech-git/we-party-web-app/pull/49) (mergeado) · Parte 6: [#50](https://github.com/we-tech-git/we-party-web-app/pull/50) (mergeado) · Parte 7: [#51](https://github.com/we-tech-git/we-party-web-app/pull/51) | **Parte 1/N** ✅ FAQ. **Parte 2/N** ✅ banner "Novidades". **Parte 3/N** ✅ Features (achado: `ref` Vue cruzando componente, resolvido sem `defineExpose`). **Parte 4/N** ✅ — timeline "Como funciona" extraída, -191 linhas. **Parte 5/N** ✅ — seção "Descubra" extraída, -424 linhas; criado `types.ts` compartilhado para `DiscoverEventCard`. **Parte 6/N** ✅ — hero extraída (2 refs Vue usadas por `IntersectionObserver`/`ScrollTrigger`, resolvidas com `querySelector`), -178 linhas. **Parte 7/N (última!)** ✅ — "app em ação" extraída, -1.055 linhas (maior fatia); única seção com estado/GSAP próprio (handlers de mouse do mockup do celular), por isso ficou com seu próprio `onMounted`/`onUnmounted`; limpeza confirmada de CSS morto (`.btn-cta-primary`/`.btn-glow` sem consumidor). **`LandingPage.vue` decomposto por completo: 5.782 → 1.501 linhas.** Próximo: `Profile.vue`/`Feed.vue`/`NewEventDetails.vue` — cada um seu próprio PR. |
