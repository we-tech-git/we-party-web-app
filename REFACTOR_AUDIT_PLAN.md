...
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟡 Em andamento | `LandingPage.vue` partes 1-7: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45)-[#51](https://github.com/we-tech-git/we-party-web-app/pull/51) (todos mergeados) · `Profile.vue` parte 1: [#52](https://github.com/we-tech-git/we-party-web-app/pull/52) (mergeado) · `Profile.vue` partes 2-6 + bônus: PR aberto | **`LandingPage.vue` decomposto por completo: 5.782 → 1.501 linhas**, em 7 fatias — narrativa completa na seção 3.4 acima. **`Profile.vue`: 5.646 → 3.953 linhas (-30%)** — todos os **7 modais** extraídos (Seguidores/Seguindo unificados; Interesses; Solicitar Interesse; Editar Perfil; Cortar Avatar; Cortar Capa) + bônus painel de Configurações. Achado que afetou 5 modais: 2 declarações conflitantes de `.btn-cancel` (uma vazava pra cascata inteira) e uma declaração incondicional de `.modal-container` com variável CSS undefined — resolvidos dando a cada modal a versão que sempre foi "sua". **2 regressões achadas e corrigidas retroativamente** durante a própria extração (antes do PR): ícone de loading sem girar (`.mdi-spin`/`@keyframes spin` nunca duplicados) e `prefers-reduced-motion` não respeitado em nenhum modal — ambas corrigidas em todos os componentes afetados. Header/tabs/sidebar **deixados para uma próxima fatia** de propósito: CSS lá usa seletores compostos cruzando várias seções futuras (ex. `.interest-chip, .tag`), tornando a decomposição seguro só com validação visual (sem login de teste nesta sessão) — detalhes na seção 3.4. Depois: `Feed.vue`/`NewEventDetails.vue`. |
