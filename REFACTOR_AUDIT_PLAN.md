...
**Próximas fatias (não feitas ainda, ordem sugerida):** seções restantes de `LandingPage.vue` (hero, discover, features, app-showcase, how-it-works, updates-teaser) — cada uma seu próprio PR, mesma checagem de dependência antes de extrair — depois `Profile.vue`, `Feed.vue`, `NewEventDetails.vue`, na ordem já definida acima.
