...
| 2 — Tokens visuais únicos | 🟡 Em revisão | [#42](https://github.com/we-tech-git/we-party-web-app/pull/42) | Escopo reduzido a só `--color-*` (decisão do time — ver Fase 2 e C1b): `tokens.css` com conteúdo real, importado em `main.css` pela primeira vez; `css-variables.css` nunca era carregado em lugar nenhum (achado); `tailwind.config.ts` referencia os tokens em vez do indigo não usado; corrige de brinde 4 pontos de cor sem efeito em `Profile.vue`. `yarn lint`/`yarn test`/`yarn build` verdes; validado visualmente (404, Landingpage) — `Profile.vue` pendente de validação por quem tem login. |
