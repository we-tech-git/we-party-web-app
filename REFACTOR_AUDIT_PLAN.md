...
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟡 Em andamento | `LandingPage.vue` partes 1-7: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45)-[#51](https://github.com/we-tech-git/we-party-web-app/pull/51) (todos mergeados) · `Profile.vue` parte 1: [#52](https://github.com/we-tech-git/we-party-web-app/pull/52) | **`LandingPage.vue` decomposto por completo: 5.782 → 1.501 linhas**, em 7 fatias (FAQ, Novidades, Features, Como Funciona, Descubra, Hero, App em Ação) — narrativa completa de cada parte fica só na seção 3.4 acima (tabela não repete). **`Profile.vue` (5.646 linhas) em andamento** — decomposição por modal, não por seção (7 modais quase idênticos no arquivo; estratégia completa na seção 3.4). **Parte 1/N** ✅ — modais de Seguidores/Seguindo unificados em 1 componente (`ProfileFollowListModal.vue` + prop `variant`), -234 linhas; achado de CSS morto (`.follow-modal-avatar img`, nunca aplicava por causa do `<span>` raiz do `UserAvatar`). Validação visual não foi possível (sem login de teste) — coberta por revisão manual linha a linha. Restam: interesses (gerenciar + solicitar), editar perfil, cortar avatar/capa, painéis de tab — depois `Feed.vue`/`NewEventDetails.vue`. |
