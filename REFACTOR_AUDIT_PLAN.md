...
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟡 Em andamento | Parte 1: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45) (mergeado) · Parte 2: [#46](https://github.com/we-tech-git/we-party-web-app/pull/46) (mergeado) · Parte 3: [#47](https://github.com/we-tech-git/we-party-web-app/pull/47) | **Parte 1/N** ✅ FAQ (achou/corrigiu bug de `.container`). **Parte 2/N** ✅ banner "Novidades", -101 linhas. **Parte 3/N** ✅ — seção de Features extraída pra `LandingFeaturesSection.vue`, -257 linhas (-183 CSS); achado novo: um `ref` Vue (não só classe CSS) cruzava a fronteira do componente, resolvido sem `defineExpose` (a função só faz `querySelectorAll`, passou a receber a raiz da página). Validado via `getComputedStyle` em desktop+mobile, correto de primeira. Restam: hero, discover, app-showcase, how-it-works de `LandingPage.vue`, depois `Profile.vue`/`Feed.vue`/`NewEventDetails.vue` — cada um seu próprio PR. |
