...
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟡 Em andamento | `LandingPage.vue` partes 1-7: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45)-[#51](https://github.com/we-tech-git/we-party-web-app/pull/51) (todos mergeados) · `Profile.vue` parte 1: [#52](https://github.com/we-tech-git/we-party-web-app/pull/52) (mergeado) · `Profile.vue` partes 2-6 + bônus: [#53](https://github.com/we-tech-git/we-party-web-app/pull/53) (mergeado) · `Feed.vue` parte 1: PR aberto | **`LandingPage.vue` decomposto por completo: 5.782 → 1.501 linhas**, em 7 fatias — narrativa completa na seção 3.4 acima. **`Profile.vue`: 5.646 → 3.953 linhas (-30%)** — todos os **7 modais** extraídos + bônus painel de Configurações; achado de 2 declarações `.btn-cancel`/`.modal-container` conflitantes resolvido; **2 regressões achadas e corrigidas retroativamente antes do PR** (ícone de loading sem girar, `prefers-reduced-motion` não respeitado). Header/tabs/sidebar deixados para depois (CSS com seletores compostos cruzando seções, ver seção 3.4). **`Feed.vue` (2.740 linhas) em andamento** — **Parte 1** ✅: FAB + bottom sheet de tendências mobile extraídos (`FeedTrendingMobile.vue`), -327 linhas. **Achado de produto**: painel de filtros (categoria/data) é código morto confirmado — `filterOpen` nunca é setado `true` em lugar nenhum do código-base, ~140 linhas de template + CSS nunca renderizam; não removido nem religado (decisão de produto, fora de escopo), registrado pro time decidir. Resto do arquivo (breadcrumb/abas/grid de eventos) fica pra depois — tudo acoplado ao mesmo estado central de paginação/busca, sem ganho real em separar agora. Depois: `NewEventDetails.vue`. |
