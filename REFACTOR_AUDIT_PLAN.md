...
**Próximas fatias (não feitas ainda, ordem sugerida):** seções restantes de `LandingPage.vue` (hero — maior risco, acoplada ao estado `navSolid`/`headerEl` do pai para o efeito de header sólido no scroll — e app-showcase, que depende de `discoverEvents`/`DiscoverEventCard` já movido para `types.ts`) — cada uma seu próprio PR, mesma checagem de dependência + classes-base + refs-que-cruzam-componente antes de extrair — depois `Profile.vue`, `Feed.vue`, `NewEventDetails.vue`, na ordem já definida acima.
