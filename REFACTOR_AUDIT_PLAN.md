...
- **Escala real, explícita:** decompor os 4 arquivos (3.707+5.782+2.740+2.234 linhas ainda hoje) num PR só seria exatamente o "big bang" que o princípio da seção 4.5/2.9 pede pra evitar. Fase 5 vira **uma série de PRs pequenos**, um pedaço por vez, cada um com seu próprio checkpoint de teste/visual — não uma fase de PR único como as anteriores.

**Parte 1 (`LandingPage.vue` — FAQ) ✅ implementada, PR em validação:**
- Extraída a seção de FAQ inteira (template + estado `faqOpen`/`toggleFaq` + dado `faqs` + CSS, incluindo responsivo) para `src/components/modules/Landingpage/LandingFaqSection.vue`. Escolhida como primeira fatia por auditoria própria: única seção 100% autocontida (sem dependência de outro estado da página), confirmado por grep antes de extrair.
- `.section-title`/`.section-description`/`.gradient-text` (heading genérico usado por 13 pontos em várias seções) foram **duplicados**, não movidos — ainda são usados pelas seções não extraídas. Duplicar 3 regras pequenas agora é mais simples e seguro do que criar um componente de heading compartilhado pra 1 único consumidor (regra 2.9, sem abstração antes da repetição real). Reavaliar quando uma 2ª seção for extraída.
- Animações GSAP/ScrollTrigger que miram `.faq-*` por seletor CSS (`gsap.from('.faq-intro', ...)`) **não precisaram mudar** — continuam funcionando porque miram o DOM global, não uma referência Vue, e o filho monta antes do `onMounted` do pai rodar. Confirmado visualmente (scroll + clique na pergunta, sem "GSAP target not found" no console).
- `LandingPage.vue`: 3.947 → 3.707 linhas (-240); CSS: 2.485 → 2.307 (-178).
- Validado com os 22 testes da Fase 4 (verdes, sem alterar nenhum), `yarn lint`/`yarn build` verdes, e checagem visual via browser (FAQ abre/fecha, layout idêntico ao anterior).
- **Bug encontrado na revisão do PR, corrigido no mesmo PR:** a seção renderizou sem centralizar (grudada na esquerda, em qualquer largura de tela). Causa: a classe **base** `.container` (`max-width` + `margin: 0 auto` + padding responsivo, definida uma única vez em `LandingPage.vue` e usada por toda seção da página) não foi duplicada — só o override específico `.faq-v2 .container` tinha sido copiado. **Lição pras próximas fatias:** antes de extrair uma seção, checar não só as classes exclusivas dela, mas toda classe "base/genérica" (`.container`, `.section-title` etc.) que o template da seção referencia — `grep` pela classe no restante do arquivo não basta sozinho; precisa confirmar que TODA classe usada no trecho extraído tem uma regra correspondente (própria ou duplicada) no novo componente. Corrigido e revalidado via `getComputedStyle` no browser (desktop: `max-width:1320px`/centralizado; mobile ~500px: largura total com padding, grid em coluna única).

**Parte 2 (`LandingPage.vue` — Novidades/banner) ✅ implementada, PR em validação:**
- Extraída a seção "Novidades" (banner estático de convite pra página de updates, sem estado próprio) para `src/components/modules/Landingpage/LandingUpdatesTeaser.vue`. Escolhida por ser a mais simples das restantes (sem estado, sem dado dinâmico).
- Aplicada a lição da parte 1: mapeada **toda** classe do template contra o CSS restante antes de extrair, não só as exclusivas da seção. Achado: `.container` (de novo) e `.btn-cta-primary`/`.btn-glow` (compartilhado com a seção "discover", ainda não extraída) também precisavam ser duplicados — feito desta vez já na primeira tentativa, sem quebrar em produção.
- 3 blocos de media query misturavam seletores da seção com os de outras seções (`.pwa-banner`, `.btn-pwa-install`, `.how-it-works-v2` etc.) — splitados: só a parte de `.updates-*` migrou, o resto ficou em `LandingPage.vue`.
- Efeito de "botão magnético" (JS, `setupMagneticButtons`) e o `gsap.from('.updates-banner', ...)` não precisaram mudar — mesma razão da parte 1 (miram o DOM global a partir do elemento raiz da página, não uma referência Vue).
- `LandingPage.vue`: 3.707 → 3.606 linhas (-101); CSS: 2.307 → 2.223 (-84).
- Validado com os 22 testes da Fase 4 (verdes), `yarn lint`/`yarn build` verdes, e **validação visual via `getComputedStyle` no browser** (não só olhar a tela) em desktop (`.container` com `max-width:1200px`, centralizado) e mobile ~500px (`.updates-banner` vira coluna no breakpoint certo) — confirmado sem o bug da parte 1.

**Parte 3 (`LandingPage.vue` — Features) ✅ implementada, PR em validação:**
- Extraída a seção de features (grid com 1 destaque + 3 secundários) para `src/components/modules/Landingpage/LandingFeaturesSection.vue`.
- **Achado novo, diferente das partes 1 e 2:** o grid tinha `ref="featuresGridEl"` (uma referência Vue de verdade, não só classe CSS) usado por `setupFeaturesSpotlight` (efeito de spotlight que segue o mouse). Como essa função só faz `gridEl.querySelectorAll(...)` internamente, resolvido **sem** `defineExpose`: o `LandingPage.vue` passou a chamar `setupFeaturesSpotlight(landingEl.value)` (a raiz da página, já usada pelo mesmo motivo em `setupMagneticButtons`) em vez do ref específico do grid.
- `.container`/`.section-title`/`.gradient-text`/`.section-header` (compartilhados com "app-showcase" e "how-it-works", ainda não extraídas) duplicados de novo — checagem de classes-base já virou rotina, não penduricalho.
- `LandingPage.vue`: 3.606 → 3.349 linhas (-257); CSS: 2.223 → 2.040 (-183).
- Validado com os 22 testes da Fase 4 (verdes), `yarn lint`/`yarn build` verdes, e `getComputedStyle` no browser em desktop (1600px: grid 2 colunas, `.container` centralizado em 1320px) e mobile (~500px: grid 1 coluna, `.feature-main` com `min-height:280px`) — ambos corretos já na primeira tentativa.

**Parte 4 (`LandingPage.vue` — Como funciona) ✅ implementada, PR em validação:**
- Extraída a seção de timeline "Como funciona" (4 passos) para `src/components/modules/Landingpage/LandingHowItWorksSection.vue`. Seção autocontida (dado `howItWorks` + template, sem refs cruzando fronteira).
- Achado extra desta vez: `.logo-text` (usado dentro do `<h2>`) é **mais uma** classe compartilhada (header, app-showcase, how-it-works) — vira rotina confirmar cada `<span>`/elemento do heading contra o CSS restante, não só as classes do container do grid.
- `LandingPage.vue`: 3.349 → 3.158 linhas (-191); CSS: 2.040 → 1.885 (-155).
- Validado com os 22 testes da Fase 4 (verdes), `yarn lint`/`yarn build` verdes, e `getComputedStyle` no browser em desktop (`.container` em 1320px, linha viva visível, 4 linhas da timeline) e mobile ~500px (linha viva escondida, linhas em coluna, número gigante de fundo escondido) — corretos de primeira.

**Parte 5 (`LandingPage.vue` — Descubra) ✅ implementada, PR em validação:**
- Extraída a seção "Descubra" (badge + título + texto + CTA + avatares + card de contador ao vivo + grid de 3 cards de evento) para `src/components/modules/Landingpage/LandingDiscoverSection.vue`.
- Criado `src/components/modules/Landingpage/types.ts` para a interface `DiscoverEventCard` — deixou de ser exclusiva da seção "discover" porque "app-showcase" (ainda não extraída) também lê `discoverEvents[0]` para o preview do celular. `discoverEvents`/`usersOnline` continuam vivendo em `LandingPage.vue` por esse motivo, passados como props.
- `discoverCardsEl` (ref Vue usado só por `setupDiscoverCardsInteractivity`, que faz apenas `querySelectorAll` internamente) foi removido; a chamada passou a usar `landingEl.value!` (a raiz da página), mesmo padrão já usado em `setupMagneticButtons`/`setupFeaturesSpotlight` — sem `defineExpose`.
- `.container`/`.btn-cta-primary`/`.btn-glow`/`.discover-avatars`+`.avatar-dot`/`.avatar-1/2/3` duplicados de propósito (compartilhados com "app-showcase", ainda não extraída); `.avatar-label` migrou junto por ser exclusivo de "discover".
- `LandingPage.vue`: 3.158 → 2.734 linhas (-424); novo componente com 553 linhas.
- Validado com os 22 testes (verdes), `yarn lint`/`yarn build` verdes, e checagem no browser (`getComputedStyle`): `.container` centralizado em 1200px, grid virando 1 coluna e padding correto (20px) em mobile (~500px — limitação da ferramenta de automação impediu forçar >768px nesta rodada, mas o CSS foi copiado 1:1 do bloco original e a lição da parte 1 — checar `.container`/classes-base — foi reaplicada).
- **Investigação registrada (não é bug):** durante a validação, o contador `.discover-live-number` aparecia sempre como "+0" e `[class*="event-mock"]` retornava 0 elementos. Confirmado que **não é regressão**: (a) o preview `event-mock` só renderiza quando `activeShowcase === 2` (aba do carrossel do "app-showcase", não é a aba padrão); (b) o "+0" reproduz **igual** no código anterior à extração (parte 4, sem nenhuma mudança) — é o tween do GSAP (`gsap.to({val:0}, {val: targetUsersOnline, ...})`) sendo throttled pelo `requestAnimationFrame` da aba em background da ferramenta de automação, não um problema do app. Confirmado via `git stash` rodando o mesmo teste no código-base antes da extração.

**Parte 6 (`LandingPage.vue` — Hero) ✅ implementada, PR em validação:**
- Extraída a seção "hero" (vídeo de fundo, wordmark/tagline com `GradientText`, indicador de scroll) para `src/components/modules/Landingpage/LandingHeroSection.vue`. Era a fatia de maior risco por causa de 2 refs Vue de verdade (`heroSection`/`heroVideo`, não só CSS) usadas fora do template: `IntersectionObserver` (efeito de header sólido no scroll, controlado por `navSolid`) e `ScrollTrigger`/`.pause()` do vídeo em `prefers-reduced-motion`.
- Resolvido sem `defineExpose`, mesmo padrão das fatias anteriores: como esses 3 usos só precisam de um `Element`/`HTMLVideoElement` (não de uma ref Vue), o pai passou a fazer `landingEl.value.querySelector('.hero'/'.hero-video')`, e o `ScrollTrigger` passou a usar o seletor `'.hero'` diretamente como `trigger`.
- O clique no indicador de scroll chamava `goToSection` do pai (que também fecha o menu mobile antes de rolar) — resolvido com `defineEmits`: o componente emite `go-to-section` e o pai decide o que fazer, sem duplicar a lógica.
- `GradientText` deixou de ser usado em `LandingPage.vue` (só a hero usava) — import removido de lá.
- `.container` duplicado de propósito (compartilhado com "app-showcase", ainda não extraída).
- `LandingPage.vue`: 2.734 → 2.556 linhas (-178); novo componente com 245 linhas.
- Validado com os 22 testes (verdes), `yarn lint`/`yarn build` verdes, `getComputedStyle`/conteúdo no browser (wordmark/tagline/vídeo/indicador presentes, `.container` em 1200px), e — pelo mesmo motivo da parte 5 — confirmado via `git stash` que o comportamento de scroll suave/`header-solid` não mudou: reproduzem igual no código-base sem a extração (mesma limitação de `requestAnimationFrame`/scroll suave em aba de automação em segundo plano). O disparo do evento `go-to-section` → `goToSection` → `scrollIntoView` foi confirmado diretamente (interceptando `Element.prototype.scrollIntoView` antes do clique).

**Parte 7 (`LandingPage.vue` — App em Ação) ✅ implementada, PR em validação — última fatia de `LandingPage.vue`:**
- Extraída a última seção ("app em ação": tabs de screenshots, mockup 3D do celular com tilt ao mouse/flutuação/entrada animada, e o banner de instalação do PWA) para `src/components/modules/Landingpage/LandingAppShowcaseSection.vue`.
- **Diferente das 6 fatias anteriores:** o mockup do celular tem estado real (refs + closures do GSAP como `badgeConfirmedX`/`phoneFloatTl`) usado pelos próprios handlers de mouse do template (`@mousemove`/`@mouseleave`) — não dava pra resolver só com `landingEl.value.querySelector(...)` porque os handlers precisam dos MESMOS refs que o setup do GSAP criou. Por isso essa seção — e só essa — ficou com seu próprio `onMounted`/`onUnmounted` (mini gsap.context local), em vez de deixar toda a inicialização centralizada no pai.
- `discoverEvents` (preview do card no tab "Detalhes do Evento") e `canInstall` (visibilidade do banner PWA) continuam vindo do pai via prop. `usePwaInstall()` não é singleton em todos os campos — `showIOSInstructions` é um `ref` novo por chamada — então chamar o composable de novo no componente criaria um ref diferente do que o modal de instruções do iOS (que fica no pai) observa; resolvido com `defineEmits('install-app')` em vez de chamar `usePwaInstall()` de novo.
- Achado extra: `.btn-cta-primary`/`.btn-glow` e `.discover-avatars`/`.avatar-*` estavam duplicados no pai desde a parte 5 "por precaução" (a nota dizia que "app-showcase" também usava) — checando de verdade, só `.discover-avatars`/`.avatar-*` eram usados (pelo preview do tab "Detalhes do Evento"); `.btn-cta-primary`/`.btn-glow` já não tinham nenhum consumidor no arquivo (hero/discover, os únicos que usavam, já tinham suas próprias cópias) — removidos do pai como limpeza confirmada, não suposição.
- `LandingPage.vue`: 2.556 → 1.501 linhas (-1.055, a maior redução de uma fatia até aqui); novo componente com 1.154 linhas.
- Validado com os 22 testes (verdes), `yarn lint`/`yarn build` verdes, conteúdo/`getComputedStyle` no browser (3 tabs, títulos corretos, mockup do celular presente, iframe do feed, `.container` em 1320px, `.avatar-dot`/`.avatar-1` com o CSS duplicado aplicado corretamente). Troca de tab e tilt ao mouse não completavam visualmente na janela em segundo plano da ferramenta de automação (mesma limitação de `requestAnimationFrame`/transições CSS já documentada nas partes 5-6) — confirmado via `git stash`, rodando exatamente a mesma checagem no código-base sem a extração, que reproduz idêntico: não é regressão.

**`LandingPage.vue` totalmente decomposto** — de 5.782 linhas originais (diagnóstico inicial) para 1.501 linhas, dividido em 8 componentes de seção (`LandingHeroSection`, `LandingDiscoverSection`, `LandingFeaturesSection`, `LandingHowItWorksSection`, `LandingFaqSection`, `LandingUpdatesTeaser`, `LandingAppShowcaseSection`) + `AppFooter`/`AppHeader` (Fase 1). Próximo: `Profile.vue`, depois `Feed.vue`, `NewEventDetails.vue`, na ordem já definida acima — cada um seu próprio PR.

#### `Profile.vue` (5.646 linhas antes desta rodada) — estratégia de decomposição

Diferente de `LandingPage.vue` (seções de scroll), `Profile.vue` é dominado por **modais quase idênticos**: `Teleport to="body"` + `Transition name="modal"` + overlay, repetido **7 vezes** (Editar Perfil, Cortar Avatar, Cortar Capa, Gerenciar Interesses, Solicitar Novos Interesses, Seguidores, Seguindo) — já registrado no diagnóstico (seção "Modal/overlay": 7 arquivos reimplementam esse esqueleto; `Modal`/`ModalShell` de base fica como **backlog**, mesma decisão do `Button`, não criado agora). A decomposição, então, é por modal (do mais isolado ao mais acoplado), não por "seção visual":

1. **Parte 1** ✅ — Modais de Seguidores/Seguindo (95% idênticos — 1 componente com prop `variant`).
2. Modal de Gerenciar Interesses.
3. Modal de Solicitar Novos Interesses.
4. Modal de Editar Perfil (mais acoplado: `ConfirmDialog` de descarte, triggers de upload de avatar/capa).
5. Modais de Cortar Avatar / Cortar Capa (usam `ImageCropper`, upload real).
6. Painéis de tab (badges/curtidos/confirmados/configurações) e o card de cabeçalho do perfil — deixados por último porque são o conteúdo "principal" da página, mais acoplados ao estado do componente do que os modais.

Cada modal já tem suas funções de `open`/`close`/`fetch` agrupadas com comentários `── Nome ──` no `<script>` — facilita achar os limites de cada fatia. `.modal-overlay`/`.modal-container`/`.modal-header`/`.modal-close`/`.modal-body`/transições `.modal-enter-*`/`.modal-leave-*` são a classe-base "genérica" (equivalente ao `.container` da Landingpage) — **duplicada em cada modal extraído**, não movida pra um componente compartilhado (mesma decisão do backlog `Modal`/`ModalShell`). Ponto de atenção específico deste arquivo: `.modal-overlay` **redeclara variáveis CSS** (`--color-primary` etc.) porque o modal é teleportado pra fora de `.profile-page-layout` — mas a redeclaração já existente no arquivo **não cobre todas** as variáveis usadas dentro dos modais (`--color-text-muted`/`--transition-fast`/`--radius-full` ficam undefined mesmo hoje, antes de qualquer extração — é o mesmo tipo de achado do C1b do diagnóstico). Decisão: preservar esse comportamento (bug incluído) em cada extração, não corrigir silenciosamente por fora do escopo desta fatia.

**Parte 1 (`Profile.vue` — Modais de Seguidores/Seguindo) ✅ implementada, PR em validação:**
- Extraídos os modais de Seguidores e Seguindo — que eram ~95% o mesmo template/CSS (só diferindo no título, ícone/texto de vazio, e a lógica do botão: em "seguidores" alterna Seguir/Seguindo por pessoa, em "seguindo" é sempre "Deixar de seguir") — para **1 componente só**, `src/components/modules/Profile/ProfileFollowListModal.vue`, com prop `variant: 'followers' | 'following'`. Repetição real confirmada dentro do próprio arquivo (não especulativa) — mesmo critério que já criou o `EventMiniCard` na Fase 3b, diferente da abordagem "1 componente por seção" da Landingpage (lá as seções eram todas visualmente/estruturalmente diferentes entre si).
- Criado `src/components/modules/Profile/types.ts` com a interface `FollowUser` (compartilhada entre `Profile.vue` e o novo componente).
- `toggleFollowUser`/`fetchFollowersList`/`fetchFollowingList` continuam no pai: `toggleFollowUser` também é usado pela lista de recomendações (fora dos modais) e mexe em `followStats` do pai — o componente só emite `toggle-follow`. `goToProfile` (via `useUserNavigation`) é seguro chamar de novo dentro do componente: o composable só lê router/auth, não guarda estado próprio por chamada (diferente do `usePwaInstall` da Landingpage, que tinha esse cuidado com `showIOSInstructions`).
- Achado: `.follow-modal-avatar img`/`.avatar-placeholder-modal` no CSS original já estavam mortas (os modais usam o componente `UserAvatar`, que renderiza um `<span>` como raiz — o `<img>` interno não recebe o atributo de escopo do `<style scoped>` do pai, então essas regras nunca aplicavam) — não duplicadas no novo componente.
- `Profile.vue`: 5.646 → 5.412 linhas (-234); novo componente com 349 linhas.
- Validado com os 3 testes de `Profile.spec.ts` (+ os outros 19 da Fase 4, total 22 verdes), `yarn lint`/`yarn build` verdes. **Validação visual no browser não foi possível** (rota `/private/profile` exige login, sem credenciais de teste disponíveis nesta sessão — mesma limitação já registrada na Fase 2) — conferido por revisão manual linha a linha do template/CSS extraído contra o original, e checagem específica de que nenhuma classe `.follow-modal-*` sobrou usada em outro lugar do arquivo antes de remover do pai.

**Partes 2-6 (`Profile.vue` — os outros 6 modais) ✅ implementadas, mesmo PR:**
- **Parte 2** — Modal de Gerenciar Interesses → `ProfileInterestsModal.vue`. Busca/sugestões/lista temporária de interesses continuam no pai (`tempUserInterests` só persiste na API quando "Concluído" é clicado); `search-query` segue convenção `update:searchQuery` porque o pai também lê o valor fora do v-model (`openRequestModal` usa o texto buscado como sugestão inicial).
- **Parte 3** — Modal de Solicitar Novo Interesse → `ProfileRequestInterestModal.vue`. Estado (`newInterestName`/`pendingInterests`) e chamada de API continuam no pai, mesmo padrão.
- **Parte 4** — Modal de Editar Perfil → `ProfileEditModal.vue`, a fatia mais acoplada: `editForm`/`saving`/`pendingAvatarFile`/`pendingBannerFile`/`discardPendingImages`/`ConfirmDialog` de descarte continuam no pai (compartilhados com os modais de recorte). Os `<input type="file">` ocultos moraram sempre dentro deste modal — diferente das fatias 1-3, aqui o evento emitido carrega o próprio elemento `<input>` (`avatar-selected`/`banner-selected`) porque o pai precisa dele pra resetar `input.value` quando o recorte é cancelado (permite reescolher o mesmo arquivo).
- **Partes 5/6** — Modais de Cortar Avatar/Capa → `ProfileCropModal.vue`/`ProfileBannerCropModal.vue` (mantidos separados, não viraram 1 componente com `variant`: proporção, formato — círculo vs retângulo —, tamanho de saída e até o uso de `t()` i18n divergem de verdade entre os dois — o de capa nunca foi traduzido, texto 100% hardcoded em português, achado registrado mas fora de escopo corrigir agora). Cada um só pega o blob recortado do `ImageCropper` e emite `confirmed`; o pai decide nome do arquivo e prévia local.
- **Achado que afeta as 5 fatias (2-6):** havia **2 declarações conflitantes de `.btn-cancel`** no `Profile.vue` original — uma pensada pro par Interesses/Solicitar (`padding: 0.75rem 2rem`), outra pro modal de Editar Perfil (`padding: 0.6rem 1.25rem`) — a 2ª "vencia" a cascata pra **todos** os modais do arquivo, por vir depois no CSS. Resolvido dando a cada modal extraído a versão de `.btn-cancel` que ele **sempre deveria ter tido**: Interesses/Solicitar ficaram com a 1ª versão (agora correta, sem vazamento), Editar Perfil/Cortar Avatar/Cortar Capa com a 2ª (que já era a que valia pra eles, só que agora sem depender de ordem de declaração). Mesmo tipo de achado num 2º ponto: `.modal-container` tinha uma declaração incondicional (`max-width: 100%`, `border-radius: var(--radius-lg)`, `--radius-lg` também undefined no escopo teleportado, mesmo padrão do C1b) sem nenhum `@media` — removida; a versão que sobrou (520px, cantos de 20px de verdade) é a única usada agora.
- **`Profile.vue` está com todos os 7 modais extraídos** — o esqueleto genérico (`.modal-overlay`/`.modal-close`/transições) não tem mais nenhum consumidor no arquivo original, cada componente carrega sua própria cópia.

**Bônus — Painel de Configurações (`ProfileSettingsPanel.vue`) ✅ implementado, mesmo PR:**
- 1ª fatia do **conteúdo principal** da página (fora dos modais): painel "Configurações" da aba de tabs, autocontido. `notificationsEnabled` segue convenção `update:modelValue`; `edit-profile`/`logout` continuam no pai.

**2 regressões achadas e corrigidas retroativamente nas partes 1-6 (achadas só durante a extração desta fatia, antes de qualquer PR ser aberto):**
1. **Ícone de loading parado:** `.mdi-spin`/`@keyframes spin` (a animação de rotação do ícone `mdi-loading` usado em 5 dos 6 modais durante save/upload/submit) nunca tinha sido duplicada em nenhum componente extraído — como `<style scoped>` não atravessa fronteira de componente, o ícone aparecia mas **não girava** em nenhum dos modais das partes 2-6 (e ninguém tinha notado ainda porque a validação visual não é possível nesta sessão, ver acima). Corrigido adicionando `.mdi-spin`/`@keyframes spin` em todos os 5 componentes que usam o ícone.
2. **Acessibilidade (`prefers-reduced-motion`) não respeitada:** a regra `@media (prefers-reduced-motion: reduce) { *, *::before, *::after { animation-duration: 0.01ms !important; ... } }` do `Profile.vue` original também nunca tinha sido duplicada — pessoas com essa preferência ativada não teriam a transição de entrada/saída dos modais nem o spin do loading desacelerados. Corrigido adicionando a mesma regra em todos os 6 componentes de modal.
- Essas 2 são o tipo exato de achado que a Fase 5 da Landingpage já tinha ensinado a procurar (classes "genéricas"/globais que um componente usa mas não define) — a diferença é que lá eram classes visuais (`.container`), aqui uma é uma animação (`mdi-spin`) e a outra uma media query de acessibilidade — ambas fáceis de esquecer porque não aparecem no grep óbvio de "classes usadas no template", só aparecem ao caçar toda classe/seletor usado dentro de cada `<style scoped>` novo contra o restante do arquivo original.

**Decisão de escopo — onde esta fatia da Fase 5 parou:** `Profile.vue` caiu de **5.646 para 3.953 linhas (-30%)**, mas o card de cabeçalho do perfil, as abas de curtidos/confirmados e a sidebar (interesses + recomendações) **não foram decompostos nesta rodada**. Investigando o CSS restante antes de decidir, ficou claro que ele é qualitativamente diferente do dos modais: várias regras "de polimento" (hover, ripple de botão, focus-visible, transições) usam **seletores compostos que misturam classes de seções diferentes** na mesma declaração — ex. `.interest-chip, .tag { transition: ... }` (chip do cabeçalho + tag da sidebar), `.edit-btn, .share-btn, .show-more-btn, .empty-action { ...ripple... }` (botões do cabeçalho + botões dos painéis de evento). Decompor essas seções exigiria fatiar cada seletor composto e duplicar a declaração em múltiplos componentes novos — o mesmo princípio já usado nos modais, só que numa escala e com uma malha de dependências visual mais difícil de auditar com segurança sem conseguir validar visualmente no browser (sem login de teste nesta sessão). Diante do achado real de 2 regressões silenciosas nas partes mais simples (modais, com CSS bem mais contido), o custo/risco de continuar sem conseguir *ver* o resultado ficou alto demais pra decisão unilateral — fica registrado aqui como próxima fatia, com o aviso específico sobre os seletores compostos pra quem pegar essa parte depois.

#### `Feed.vue` (2.740 linhas antes desta rodada)

**Parte 1 (`Feed.vue` — FAB + Bottom Sheet de Tendências mobile) ✅ implementada:**
- Extraído o botão flutuante (FAB) + bottom sheet de tendências, exclusivos do layout mobile, para `src/components/modules/Feed/FeedTrendingMobile.vue`. Diferente dos modais de `Profile.vue`, esta seção não tinha nenhum "esqueleto genérico" compartilhado com outra parte do arquivo — CSS totalmente autocontido, extração direta.
- `displayedTrends` continua no pai (também alimenta o `FeedTrendsPanel.vue`, painel de tendências do desktop, já existente como componente separado antes desta fase). `goToTrendingEvent` (guarda de `guestMode`/`requireLogin` + navegação) migrou inteiro pro componente novo — mesmo padrão já usado em `FeedTrendsPanel.vue` (`goToMainEvent`), inclusive o import direto de `router` (`import router from '@/router'`) em vez de `useRouter()`, pra ficar consistente com o componente irmão que já existia.
- `Feed.vue`: 2.740 → 2.413 linhas (-327); novo componente com 384 linhas.
- Validado com os 22 testes (verdes), `yarn lint`/`yarn build` verdes. Sem classes genéricas/globais faltando (checagem específica feita depois do achado do `mdi-spin` em `Profile.vue`: este arquivo não tem nenhum ícone com `mdi-spin` nem `@media (prefers-reduced-motion)`/`(prefers-contrast)` — confirmado por grep no arquivo inteiro antes de fechar a fatia).

**Achado importante (não é uma fatia de decomposição — é um achado de produto/dead code):** o painel de filtros da busca (categoria + data, `<div v-if="filterOpen" class="filter-panel">`, ~140 linhas de template + ~33 regras de CSS incluindo media queries) **nunca é exibido** — `filterOpen` é inicializado `ref(false)` e **não existe nenhum lugar no código-base inteiro** (`grep -rn "filterOpen" src/`) que o altere para `true`. Ou é uma feature que ficou pela metade (o botão que abriria o painel foi removido/nunca conectado) ou o painel foi substituído por outra coisa e o código não foi limpo. Não foi removido nem "consertado" (ligar o toggle seria mudança de comportamento/produto, fora do escopo de um refactor) — fica registrado aqui para o time decidir: religar (existe um botão de filtro em algum lugar do design que deveria setar `filterOpen.value = true`?) ou remover como código morto.

**Decisão de escopo:** parada aqui por ora. O que resta em `Feed.vue` (breadcrumb, abas, grid de `FeedCard` com load-more/fim-de-resultados/vazio) é todo acoplado ao mesmo estado central de busca/paginação (`displayedItems`, `loading`, `hasMore`, `activeNav`, `isSearching` etc.) — diferente do painel de filtros (isolado, mas morto) e do FAB de tendências (isolado e vivo), decompor essas seções não reduziria acoplamento real, só moveria template pra outro arquivo com uma superfície grande de props/emits. Não foi um "não dava pra fazer" — foi um "o ganho não paga o risco" nesta rodada.

#### `NewEventDetails.vue` (2.234 linhas antes desta rodada) — último mega-arquivo da Fase 5

Diferente de `Profile.vue`/`LandingPage.vue`, o CSS aqui é só 23% do arquivo (diagnóstico original) — a maior parte do template usa classes utilitárias estilo Tailwind (`w-5 h-5`, `flex items-center`, etc., globais, sem duplicação necessária) misturadas com um conjunto pequeno (~65 classes) de utilitários customizados próprios do arquivo (`.card`, `.btn-soft`, `.text-weparty-pink`, `.font-display` etc.) — esses sim precisam ser duplicados nos componentes extraídos, exatamente como as classes "genéricas" dos outros arquivos. Sem `@media` nem regra de `prefers-reduced-motion` no arquivo inteiro (confirmado por grep) — bem menos fragmentado que `Profile.vue`.

**Partes 1-5 ✅ implementadas — 5 seções autocontidas extraídas:**
1. **`EventFaqAccordion.vue`** — acordeão de Perguntas Frequentes. Normalização (`normalizedFaqs`) e estado de abrir/fechar (seção + cada pergunta) migraram inteiros pro componente; só recebe o array cru de FAQs da API.
2. **`EventTrendingCard.vue`** — card "O que tá rolando?" (lateral direita). A busca (`fetchTrending`/`mapTrend`, que usa `resolveAsset`/`eventsStore` compartilhados com outras partes) continua no pai; o componente só recebe a lista já mapeada e cuida do "Mostrar mais/menos" + navegação (`router.push` direto, mesmo padrão de `FeedTrendsPanel.vue`/`FeedTrendingMobile.vue`).
3. **`EventLocationPanel.vue`** — aba "Local" (mapa do Google Maps + endereço). `mapEmbedUrl`/`openMap` migraram inteiros; só recebe `venue`/`city`/`location`.
4. **`EventLineupPanel.vue`** — aba "Atrações" (grid de artistas). Puramente de exibição, só recebe `lineup`.
5. **`EventSocialBar.vue`** — barra flutuante de avatares + curtidas/confirmados, abaixo do hero. Puramente de exibição — `attendeeAvatars`/`goingCount`/`likeCount` continuam no pai (lidos também pelo hero, card de ação e barra inferior mobile, nenhum desses extraído ainda).
- `.card`/`.card-header`/`.card-icon`/`.btn-soft`/`.text-weparty-pink`/`.bg-pink-50`/`.font-display`/`.shadow-card`: classes-base compartilhadas com a aba "Informações" (a maior, ainda não extraída) — duplicadas em cada componente que precisa, mantidas no pai também (confirmado por grep antes de cada remoção).
- `NewEventDetails.vue`: 2.234 → 1.676 linhas (-558, -25%).
- Validado com os 22 testes (verdes), `yarn lint`/`yarn build` verdes. Checagem de comentários CSS balanceados (`/* ... */`) repetida em cada arquivo novo, depois do achado do bug de sintaxe da Fase 5 anterior (comentário contendo `*/` no meio do texto fecha o bloco antes da hora e quebra o build do `vite-plugin-pwa`).

**Achados de produto (não são decomposição — 3 blocos de código confirmadamente mortos, `v-if="false"` literal):**
1. **Termômetro do evento** ("calor"/lotação, `eventHeat` computed + `.heat-badge`/`.heat-track`/`.heat-fill`): `<div v-if="false" class="mb-5">` no card de ação — nunca renderiza, `eventHeat` não é lido em nenhum outro lugar.
2. **Preview de avatares alternativo** no card de ação (`<div v-if="false" ...>` logo abaixo dos botões de ação): usa `<img>` cru em vez do componente `UserAvatar`, parece um rascunho anterior à versão atual (`EventSocialBar.vue`, que já mostra os avatares de forma equivalente, ativa, mais acima na página).
3. Campos `icon`/`gradient` de `normalizedFaqs` (dentro de `EventFaqAccordion.vue`, herdados do arquivo original): calculados mas nunca renderizados no template do acordeão.
- Nenhum removido — decisão de produto/UI, fora do escopo de um refactor que promete preservar comportamento. Registrados aqui pro time decidir (remover como limpeza ou terminar de religar a feature).

**Decisão de escopo — onde parou:** a aba "Informações" (maior painel: cartão de data/countdown/grid de infos rápidas, cartão de descrição com "Ler mais", cartão do organizador com botão de seguir) não foi decomposta. São na real 3 sub-widgets diferentes dentro do mesmo painel, cada um com acoplamento próprio real: o grid de infos rápidas **escreve** em `activeTab` (clicar num item pode trocar de aba), o cartão de descrição usa um `ref` de template (`aboutEl`) pra medir se o texto estourou a altura e mostrar "Ler mais" (mediação teria que migrar junto pro componente, sem `defineExpose`, senão nenhuma referência sobra pra medir), e o cartão do organizador depende de `following`/`followLoading`/`toggleFollow`/`goToProfile`. Dava pra extrair os 3 separadamente, mas cada um exigiria seu próprio desenho de props/emits sem o benefício de reutilização que os 5 já extraídos tinham (nenhum se repete em outro lugar do app) — ficou registrado aqui como trabalho futuro, não como "impossível".

**Fase 5 concluída** para os 4 mega-arquivos originais do diagnóstico (`LandingPage.vue`, `Profile.vue`, `Feed.vue`, `NewEventDetails.vue`) — cada um teve pelo menos uma rodada de decomposição real, com `LandingPage.vue` 100% decomposto e os outros 3 com as partes mais isoladas/repetidas já extraídas e o restante (mais acoplado ao estado central de cada tela) registrado explicitamente como trabalho futuro.

### Fase 6 — Eliminar duplicação restante e consolidar estado ✅ concluída
...
- **O que mudou:**
  - **B3 — achado, não a suposição original:** `FeedCardStandalone.vue` **não** era "`FeedCard` com uma variação real" como o diagnóstico supunha — é **código morto confirmado**. `grep -rn "FeedCardStandalone" src/` (fora do próprio arquivo) não retorna nenhuma página/rota/componente ativo importando-o. Não havia "2 implementações do mesmo conceito" pra consolidar em uma só com prop de variante — havia uma implementação viva (`FeedCard.vue`) e um arquivo órfão. Movido pra `src/components/_revisar_/`, mesmo tratamento do `EventDetails.vue`/`EventView.vue` na Fase 1 (ver `_revisar_/README.md`, atualizado com a correção).
  - **D1 — consolidado o que realmente violava o princípio:** de 21 arquivos que usam `localStorage`, a maioria é dado que não é sessão (tema, rate-limit, e-mail lembrado, handoff de cadastro/recuperação de senha — fora do escopo de "sessão" do item). Achados reais de acesso direto ao token de sessão, bypassando `AuthService`/`useAuth`:
    - `src/api/index.ts` (2 pontos) e `src/api/users.ts` (3 pontos): liam `localStorage.getItem('ACCESS_TOKEN')` direto (string solta, nem usava o enum `STORAGE_KEYS`) em vez de `AuthService.getToken()` — mesmo `AuthService` já importado no primeiro arquivo. Trocado nos 5 pontos.
    - `src/api/users.ts` também tinha uma string de erro corrompida (mojibake: "autenticaÃ§Ã£o nÃ£o encontrado") num dos 3 pontos — corrigida no caminho, comparando com as 2 outras ocorrências idênticas e corretas no mesmo arquivo.
    - `src/services/socialAuth.ts`: gravava `STORAGE_KEYS.AUTH_TOKEN`/`STORAGE_KEYS.USER_ID` direto no localStorage a cada login social, **além** de já chamar `AuthService.saveAuthData(...)` (que grava `ACCESS_TOKEN`/`LOGGED_USER`, as chaves de verdade). Confirmado por grep que nenhum lugar do código-base **lê** `AUTH_TOKEN`/`USER_ID` — só `auth.ts` as limpa no logout. Eram gravação 100% redundante, provavelmente resquício de uma versão anterior do fluxo de login social. Removida.
  - **D3:** `formatDate` em `CommentsDrawer.vue` era idêntica à de `src/components/modules/Feed/commentDisplay.ts` (o próprio comentário do arquivo já apontava isso como "próximo passo"). Substituída pelo import; comentário do `commentDisplay.ts` atualizado (a menção a `EventDetails.vue`, código morto, foi corrigida, e ficou registrado que `NewEventDetails.vue` usa formatadores de propósito diferente — data/hora do próprio evento, não tempo relativo de comentário — não são duplicatas).
- **Checkpoint 3.0:** adicionado `data-testid` (`feed-card`, `feed-card-like`, `feed-card-comments-toggle`, `feed-card-share`) em `FeedCard.vue` — a única implementação viva do card de evento, confirmada nesta fase. Sub-agente no `weparty-automation` acionado logo em seguida (ver mais abaixo) para cobrir curtir/comentar/compartilhar a partir desses seletores.
- **Validação:** `yarn lint`/`yarn vitest run`/`yarn build` verdes depois de cada mudança; revisão manual de cada ponto de `localStorage` antes de decidir se era sessão ou não.
- **Resultado esperado → obtido:** uma implementação por conceito — sem card duplicado pra consolidar (já não existia de verdade), acesso a token de sessão unificado em `AuthService`/`useAuth`, `formatDate` de comentários com uma única fonte.

### Fase 7 — Documentação consolidada + gates contínuos ✅ concluída
- **Objetivo:** registrar, de uma vez e de forma organizada, tudo que foi decidido/construído nas Fases 0-6 — e impedir que o débito técnico volte a crescer. Documentação deliberadamente feita ao final: escrever regra antes de ela existir de verdade no código gera doc que já nasce errada (o mesmo problema que gerou H1/H2 no diagnóstico).
- **O que mudou:**
  - **Reorganização de `docs/` (H1):** 10 documentos históricos (`CONSOLE_ERRORS_FIX.md`, `CORRECTIONS_SUMMARY.md`, `CSS_CORRECTIONS_REPORT.md`, `GOOGLE_OAUTH_FRONTEND_INTEGRATION.md`, `IMPROVEMENTS_GUIDE.md`, `OPTIMIZATION_GUIDE.md`, `QUICK_START_SOCIAL_AUTH.md`, `SEARCH_INPUT_MAPPING.md`, `SECURITY_REPORT.md`, `UPLOAD_TEST_GUIDE.md`) movidos para `docs/archive/`, com `docs/archive/README.md` explicando o critério (relatório pontual vs. referência viva) e listando explicitamente o que **não** foi arquivado e por quê (specs de backend são contrato ativo; `RATE_LIMITING_GUIDE.md`/`SOCIAL_AUTH_GUIDE.md` são referência viva). `docs/README.md` reescrito como índice.
  - **Documentos vivos criados:**
    - [`docs/TESTING.md`](docs/TESTING.md) — onde/como escrever teste (função pura vs. componente), setup (`src/test/setup.ts`/`vuetify.ts`), comando (`yarn test`), e a divisão de responsabilidade com o `weparty-automation` (E2E).
    - [`docs/DESIGN_SYSTEM.md`](docs/DESIGN_SYSTEM.md) — catálogo de `src/components/UI/`, critério de reuso/criação, e o resultado da auditoria de duplicação da Fase 3/6 (o que foi unificado — AppHeader/AppFooter/EventMiniCard/FeedCard —, o que foi auditado e mantido separado, e o que ficou como backlog — Button, Modal/ModalShell, Chip/Badge).
    - [`src/styles/README.md`](src/styles/README.md) expandido — tabela dos arquivos de `src/styles/`, de onde vem cada categoria de valor hoje, e a ordem de decisão Vuetify → DS → Tailwind → CSS scoped (seção 2.6).
  - **`AGENTS.md` atualizado de uma vez** (não incremental, ver seção 5): comando `test`, correção do package manager (só `yarn`, não existe `package-lock.json` no repo), lista de stores corrigida (`events`/`interestPage`/`share`, sem `app`), seção "Reuso antes de criar" reescrita apontando só para `src/components/UI/` + `docs/DESIGN_SYSTEM.md` (removida a referência a `components/modules/UI/`, que não existe mais), regras novas de Header/Footer/tokens/`data-testid`, seção "Antes de abrir PR" (`type-check`/`lint`/`test`/`build`), reforço de não tocar em `openspec/`/`.opencode/`/`.claude/commands/opsx/*`/`.claude/skills/openspec-*`. Checklist de segurança mantida sem alteração.
  - **G3 — `.github/workflows/ci.yml`:** adicionado step "Testes" (`yarn test`) no job `ci`, entre `Lint` e `Type-check & Build` — antes disso o CI rodava lint + type-check + build sem nenhuma etapa de teste.
  - **B5 — `eslint.config.js`:** adicionada regra `@typescript-eslint/no-explicit-any` em nível `warn` (não `error`) — sinaliza uso novo de `any` sem quebrar `yarn lint` por causa das 197 ocorrências já existentes (legado fora do escopo desta fase; confirmado com `yarn lint` rodando com exit code 0 e os `any` existentes aparecendo como warning, não erro).
- **Dependências:** Fases 0-6.
- **Risco:** baixo.
- **Validação:** `yarn lint` (0 erros, warnings esperados de `any` legado), `yarn test` (22/22 verdes), `yarn build` (type-check + vite build + PWA precache, sem erro) rodados depois de todas as mudanças desta fase.
- **Resultado obtido:** os problemas do diagnóstico (seção 1) relacionados a documentação (H1-H3) e a ausência de gate de teste em CI (G3)/regra progressiva de `any` (B5) estão resolvidos; a documentação reflete o projeto real, não um plano.

Este documento (`REFACTOR_AUDIT_PLAN.md`) passa a ser **histórico** a partir daqui — registro do diagnóstico e das decisões tomadas nas Fases 0-7, não documentação viva a manter sincronizada. Para o estado atual do projeto, ver `docs/README.md`.
...
| 4 — Testes de regressão nos mega-arquivos | 🟢 Concluída | [#44](https://github.com/we-tech-git/we-party-web-app/pull/44) (mergeado) | 15 testes novos (Vue Test Utils), infra de teste reaproveitável (`src/test/setup.ts`, `src/test/vuetify.ts`). Achado de produto registrado: erro de "não autenticado" no `Profile.vue` nunca aparece na tela. |
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟢 Concluída (com trabalho futuro registrado) | `LandingPage.vue` partes 1-7: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45)-[#51](https://github.com/we-tech-git/we-party-web-app/pull/51) · `Profile.vue`: [#52](https://github.com/we-tech-git/we-party-web-app/pull/52), [#53](https://github.com/we-tech-git/we-party-web-app/pull/53) · `Feed.vue`: [#54](https://github.com/we-tech-git/we-party-web-app/pull/54) · `NewEventDetails.vue`: PR final desta sessão (ver rodapé) | **`LandingPage.vue` decomposto por completo: 5.782 → 1.501 linhas**, 7 fatias. **`Profile.vue`: 5.646 → 3.953 linhas (-30%)** — 7 modais + painel de Configurações; achado de `.btn-cancel`/`.modal-container` conflitantes resolvido; 2 regressões (ícone sem girar, `prefers-reduced-motion`) achadas e corrigidas antes do PR. **`Feed.vue`: 2.740 → 2.413 linhas** — FAB/bottom-sheet de tendências mobile extraído; achado de produto: painel de filtros é código morto (`filterOpen` nunca setado `true`). **`NewEventDetails.vue`: 2.234 → 1.676 linhas (-25%)** — 5 seções autocontidas extraídas (FAQ, trending, local/mapa, atrações, barra social); 3 blocos `v-if="false"` confirmados mortos (termômetro, preview alternativo de avatares, campos não usados do FAQ). Em cada arquivo, o que sobrou é o conteúdo mais acoplado ao estado central da própria tela (header/abas/sidebar do Profile, paginação/busca do Feed, aba "Informações" do NewEventDetails) — documentado como trabalho futuro na seção 3.4, não como "impossível". |
| 6 — Eliminar duplicação restante | 🟢 Concluída | PR final desta sessão (ver rodapé) | **B3**: `FeedCardStandalone.vue` era código morto confirmado (não uma variação real de `FeedCard`) — arquivado em `_revisar_/`. **D1**: 5 pontos de leitura direta de `ACCESS_TOKEN` (`api/index.ts` 2x, `api/users.ts` 3x) trocados por `AuthService.getToken()`; corrigido no caminho um mojibake numa mensagem de erro; removida gravação redundante de `AUTH_TOKEN`/`USER_ID` (nunca lidos) em `socialAuth.ts`. **D3**: `formatDate` duplicado em `CommentsDrawer.vue` substituído pelo import de `commentDisplay.ts`. **Checkpoint 3.0**: `data-testid` adicionado em `FeedCard.vue` (`feed-card`/`feed-card-like`/`feed-card-comments-toggle`/`feed-card-share`) + sub-agente `weparty-automation` acionado. |
| 7 — Documentação consolidada + gates | 🟢 Concluída | PR final desta sessão (ver rodapé) | `docs/` reorganizado (10 arquivos históricos → `docs/archive/`, `docs/README.md` reescrito); criados `docs/TESTING.md` e `docs/DESIGN_SYSTEM.md`; `src/styles/README.md` expandido; `AGENTS.md` atualizado por completo (comandos, stores, package manager, seção "Reuso antes de criar", regras de Header/Footer/tokens/`data-testid`, "Antes de abrir PR"); **G3**: `yarn test` adicionado ao CI (`.github/workflows/ci.yml`); **B5**: regra `@typescript-eslint/no-explicit-any` em `warn` no `eslint.config.js`. `yarn lint`/`yarn test`/`yarn build` verdes ao final. |
