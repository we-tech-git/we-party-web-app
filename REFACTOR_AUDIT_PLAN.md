...
**Próximas fatias (não feitas ainda, ordem sugerida):** só resta "app-showcase" em `LandingPage.vue` (depende de `discoverEvents`/`DiscoverEventCard` já movido para `types.ts`, e de `activeShowcase`/`phoneScreenEl`/`feedEmbedScale`, que ficam no pai) — seu próprio PR, mesma checagem de dependência + classes-base + refs-que-cruzam-componente antes de extrair — depois `Profile.vue`, `Feed.vue`, `NewEventDetails.vue`, na ordem já definida acima.
...
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟡 Em andamento | Parte 1: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45) (mergeado) · Parte 2: [#46](https://github.com/we-tech-git/we-party-web-app/pull/46) (mergeado) · Parte 3: [#47](https://github.com/we-tech-git/we-party-web-app/pull/47) (mergeado) · Parte 4: [#48](https://github.com/we-tech-git/we-party-web-app/pull/48) (mergeado) · Parte 5: [#49](https://github.com/we-tech-git/we-party-web-app/pull/49) (mergeado) · Parte 6: [#50](https://github.com/we-tech-git/we-party-web-app/pull/50) | **Parte 1/N** ✅ FAQ. **Parte 2/N** ✅ banner "Novidades". **Parte 3/N** ✅ Features (achado: `ref` Vue cruzando componente, resolvido sem `defineExpose`). **Parte 4/N** ✅ — timeline "Como funciona" extraída, -191 linhas (-155 CSS); achado: `.logo-text` é mais uma classe compartilhada entre 3 seções. **Parte 5/N** ✅ — seção "Descubra" extraída, -424 linhas; criado `types.ts` compartilhado para `DiscoverEventCard` (ainda usado por "app-showcase"); investigado e descartado suspeita de regressão no contador ao vivo/preview do celular (reproduz igual no código-base, é throttling de `requestAnimationFrame` da aba em background da ferramenta de automação). **Parte 6/N** ✅ — hero extraída (maior risco: 2 refs Vue de verdade usadas por `IntersectionObserver`/`ScrollTrigger`/`.pause()`, resolvidas com `querySelector` a partir de `landingEl` + `defineEmits` pro clique no indicador de scroll), -178 linhas. Só resta app-showcase em `LandingPage.vue`, depois `Profile.vue`/`Feed.vue`/`NewEventDetails.vue` — cada um seu próprio PR. |
