...
**Próximas fatias (não feitas ainda, ordem sugerida):** seções restantes de `LandingPage.vue` (hero, discover, app-showcase) — cada uma seu próprio PR, mesma checagem de dependência + classes-base + refs-que-cruzam-componente antes de extrair — depois `Profile.vue`, `Feed.vue`, `NewEventDetails.vue`, na ordem já definida acima.
...
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟡 Em andamento | Parte 1: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45) (mergeado) · Parte 2: [#46](https://github.com/we-tech-git/we-party-web-app/pull/46) (mergeado) · Parte 3: [#47](https://github.com/we-tech-git/we-party-web-app/pull/47) (mergeado) · Parte 4: [#48](https://github.com/we-tech-git/we-party-web-app/pull/48) | **Parte 1/N** ✅ FAQ. **Parte 2/N** ✅ banner "Novidades". **Parte 3/N** ✅ Features (achado: `ref` Vue cruzando componente, resolvido sem `defineExpose`). **Parte 4/N** ✅ — timeline "Como funciona" extraída, -191 linhas (-155 CSS); achado: `.logo-text` é mais uma classe compartilhada entre 3 seções. Validado via `getComputedStyle` em desktop+mobile, correto de primeira. Restam: hero, discover, app-showcase de `LandingPage.vue`, depois `Profile.vue`/`Feed.vue`/`NewEventDetails.vue` — cada um seu próprio PR. |
