...
**Próximas fatias (não feitas ainda, ordem sugerida):** seções restantes de `LandingPage.vue` (hero, discover, features, app-showcase, how-it-works) — cada uma seu próprio PR, mesma checagem de dependência + classes-base antes de extrair — depois `Profile.vue`, `Feed.vue`, `NewEventDetails.vue`, na ordem já definida acima.
...
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟡 Em andamento | Parte 1: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45) (mergeado) · Parte 2: [#46](https://github.com/we-tech-git/we-party-web-app/pull/46) | **Parte 1/N** ✅ — FAQ extraída (achou e corrigiu bug de `.container` faltando na revisão). **Parte 2/N** ✅ — banner "Novidades" extraído pra `LandingUpdatesTeaser.vue`, -101 linhas (-84 CSS); lição da parte 1 aplicada (classes-base checadas antes), validado via `getComputedStyle` em desktop+mobile, sem bug desta vez. Restam: hero, discover, features, app-showcase, how-it-works de `LandingPage.vue`, depois `Profile.vue`/`Feed.vue`/`NewEventDetails.vue` — cada um seu próprio PR. |
