...
**Próximas fatias (não feitas ainda, ordem sugerida):** seções restantes de `LandingPage.vue` (hero, discover, features, app-showcase, how-it-works, updates-teaser) — cada uma seu próprio PR, mesma checagem de dependência antes de extrair **e a checagem de classes-base acima** — depois `Profile.vue`, `Feed.vue`, `NewEventDetails.vue`, na ordem já definida acima.
...
| 5 — Decompor mega-arquivos (**multi-PR**) | 🟡 Em andamento | Parte 1: [#45](https://github.com/we-tech-git/we-party-web-app/pull/45) | **Parte 1/N** — `LandingPage.vue`: seção de FAQ extraída pra `LandingFaqSection.vue` (única seção 100% autocontida, confirmado antes de extrair). -240 linhas (-178 de CSS). 22 testes da Fase 4 seguem verdes sem alteração; `yarn lint`/`yarn build` verdes; validado visualmente (toggle + animação GSAP). Restam: outras 6 seções de `LandingPage.vue`, depois `Profile.vue`/`Feed.vue`/`NewEventDetails.vue` — cada um seu próprio PR. |
