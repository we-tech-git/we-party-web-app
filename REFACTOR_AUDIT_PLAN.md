...
| 3 — Auditoria de componentes + DS/Storybook | 🟡 Em revisão | [#43](https://github.com/we-tech-git/we-party-web-app/pull/43) | Auditoria 3a feita (tabela no plano): `EventMiniCard` migra (3 instâncias, feito), `Button`/`Modal` migram mas ficam como backlog incremental, cards de conteúdo/marketing/atalho não migram. Storybook instalado e rodando; `ShareSheet` consolidado (resolve A2); `EventMiniCard` criado + 3 pontos de uso migrados; story do `AppLoader`. `yarn lint`/`yarn test`/`yarn build` verdes; validado visualmente via Storybook. |
